...
def get_items(session: DBSession, board_id: int, account: OptionalAccount) -> ItemCollection:
...
def get_item(session: DBSession, board_id: int, item_id: int, account: OptionalAccount) -> SomeItem:
...
def add_item(session: DBSession, board_id: int, config: ItemCreate, account: CurrentAccount) -> SomeItem:
    """If the account can edit this board, add an item."""
    return convert_item( items_db.create_item(session, board_id, config, account) )

@router.put("/{item_id}", status_code=200)
def update_item(session: DBSession, board_id: int, item_id: int, config: ItemUpdate, account: CurrentAccount) -> SomeItem:
    """Updates an item on this board."""
    return convert_item( items_db.update_item(session, board_id, item_id, config, account) )

@router.delete("/{item_id}", status_code=204)
def delete_item(session: DBSession, board_id: int, item_id: int, account: CurrentAccount) -> None:
    """Deletes an item on this board."""
    items_db.delete_item(session, board_id, item_id, account)

@router.post("/todo", status_code=201)
def add_todo_item(session: DBSession, board_id: int, config: TodoItemCreate, account: CurrentAccount) -> TodoItem:
...
def update_todo_item(session: DBSession, board_id: int, todo_item_id: int, config: TodoItemUpdate, account: CurrentAccount) -> TodoItem:
...
def delete_todo_item(session: DBSession, board_id: int, todo_item_id: int, account: CurrentAccount) -> None:
    """Add an item to a todo list on this board."""
    items_db.delete_todo_item(session, board_id, todo_item_id, account)

@router.put("/pins/connect", status_code=200)
def connect_pin(session: DBSession, board_id: int, p1: int, p2: int, account: CurrentAccount) -> list[Pin]:
...
def disconnect_pin(session: DBSession, board_id: int, p1: int, p2: int, account: CurrentAccount) -> list[Pin]:
...
def create_pin(session: DBSession, board_id: int, config: PinCreate, account: CurrentAccount) -> Pin:
...
def update_pin(session: DBSession, board_id: int, pin_id: int, config: PinUpdate, account: CurrentAccount) -> Pin:
...
def delete_pin(session: DBSession, board_id: int, pin_id: int, account: CurrentAccount):
